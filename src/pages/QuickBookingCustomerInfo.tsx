...
// Schema for manual form submission (requires password)
...
// Schema for Google OAuth users (no password required)
...
export default function QuickBookingCustomerInfo() {
  ...
  // Check for Google OAuth return and restore session data
  useEffect(() => {
  ...
  // Submit function for Google OAuth users (no password)
  const handleGoogleSubmit = async () => {
    setErrors({});
    setAutoSubmitting(true);

    console.log("Google user submit started:", {
      name: formData.name,
    ...
    const result = googleUserSchema.safeParse(formData);
    if (!result.success) {
      ...
      console.error("Google user validation errors:", fieldErrors);
    ...
    try {
      ...
      console.log("Calling edge function for Google user:", {
        bookingId,
      ...
      const { data: reservationResult, error: reservationError } = await supabase.functions.invoke(
        "create-quick-booking-reservation",
        {
          body: {
            ...
            customerPassword: null, // No password for Google users
            isGoogleUser: true,
          },
        }
      );

      console.log("Edge function response (Google):", reservationResult, "Error:", reservationError);

      if (reservationError) throw reservationError;
      if (!reservationResult?.success) throw new Error(reservationResult?.error || "Failed to create reservation");

      setSubmitted(true);
      toast.success(t("bookingConfirmedGoogleUser") || "Booking confirmed! You're signed in with Google.");
    } catch (err: any) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-primary/5 to-background p-4">
      <Card className="max-w-lg w-full">
        ...
        <CardContent>
          ...
          {/* Google User Form - Shown when user is signed in via Google */}
          {isGoogleUser ? (
            <div className="space-y-4">
              <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4 mb-4">
                <div className="flex items-center gap-2 text-green-700 dark:text-green-400">
                  <CheckCircle className="h-5 w-5" />
                  <span className="font-medium">{t("signedInWithGoogle") || "Signed in with Google"}</span>
                </div>
              ...
              >
                {autoSubmitting ? (
                  <><Loader2 className="h-4 w-4 mr-2 animate-spin" /> {t("confirming") || "Confirming..."}</>
                ) : (
