...
import { AgencyPaymentStats } from '@/hooks/useAgencyPayments';
import { motion } from 'framer-motion';

interface AgencyPaymentStatsProps {
  stats: AgencyPaymentStats;
  language: 'TR' | 'EN';
}
...
const translations = {
  ...
  TR: {
    ...
    cash: 'Nakit',
    online: 'Online',
    companyDebt: 'Şirket Borcu',
    settled: 'Ödenen',
  }
};

export const AgencyPaymentStatsCard = ({ stats, language }: AgencyPaymentStatsProps) => {
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-2">
        {/* Paid / Settled */}
        <Card className="border-green-500/20 bg-gradient-to-br from-green-50/50 to-background dark:from-green-950/20">
        ...
        {/* Unpaid / Company Debt */}
        <Card className="border-amber-500/20 bg-gradient-to-br from-amber-50/50 to-background dark:from-amber-950/20">
          <CardContent className="p-3">
            <div className="flex items-center gap-2 mb-1">
              <Building2 className="h-4 w-4 text-amber-500" />
              <span className="text-xs text-muted-foreground">{t.companyDebt}</span>
            </div>
        ...
        <Card>
          <CardContent className="p-3">
            <div className="flex items-center justify-around gap-2">
              ...
              {Object.entries(stats.byProvider).filter(([p]) => p !== 'unknown').length > 0 && (
            ...
            <div className="flex flex-wrap gap-2">
              {Object.entries(stats.byCurrency).map(([currency, data]) => (
                <div key={currency} className="flex items-center gap-1.5 bg-muted/50 px-2 py-1 rounded-md">
                  ...
                  {data.unpaid > 0 && (
                    <span className="text-xs text-amber-600">-{getCurrencySymbol(currency)}{data.unpaid.toFixed(0)}</span>
                  )}
                </div>
              ))}
            </div>
