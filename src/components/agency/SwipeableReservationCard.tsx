...
import { MapPin, Calendar, Clock, User, Car, Edit, X, Eye, Phone, CreditCard } from 'lucide-react';
...
export const SwipeableReservationCard = ({ 
  ...
  const handleDragEnd = async (_: any, info: PanInfo) => {
    ...
    // Swipe right for cancel (only for certain statuses)
    else if (offset > SWIPE_THRESHOLD && onCancel && canCancel) {
      triggerHaptic('medium');
      onCancel();
    }
  };
  ...
  const canCancel = !['completed', 'cancelled', 'cancelled_by_customer', 'cancelled_by_agency', 'active'].includes(reservation.status);
  const canSwipe = canEdit || canCancel;
  ...
  return (
    <div className="relative overflow-hidden rounded-xl">
      ...
      {canCancel && (
        <motion.div 
          className="absolute inset-y-0 left-0 w-full bg-destructive flex items-center justify-start pl-6 rounded-xl"
      ...
      >
        <Card 
          className={cn(
            ...
            swipeDirection === 'right' && "border-destructive border-2"
          )}
          onClick={onView}
        >
