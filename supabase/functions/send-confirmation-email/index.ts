...
// Inline email template helpers to avoid shared module import issues
...
function getEmailFooter(lang: string = 'en'): string {
  const contactText = lang === 'tr' ? 'Sorularınız için 7/24 bize ulaşabilirsiniz' : 'Questions? We\'re available 24/7';
  return `
<tr>
  <td style="background:#1e293b;padding:25px;text-align:center;">
    <p style="color:#94a3b8;font-size:13px;margin:0 0 15px;">${contactText}</p>
...
function getTranslation(lang: string): any {
  const translations: Record<string, any> = {
    en: { reservationConfirmed: 'Reservation Confirmed' },
    tr: { reservationConfirmed: 'Rezervasyon Onaylandı' }
  };
